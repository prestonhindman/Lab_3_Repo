...
car_price_prediction = mutate(car_price_prediction, logprice = log(selling_price), 
                              logyear = log(year), logkm = log(km_driven)) |> 
  filter(fuel != "Electric")
pricemod = lm(logprice ~ . - name - year - selling_price - km_driven, data = car_price_prediction)
```
...
pricelasso = glmnet(logprice ~ . - name - year - selling_price - km_driven, data = car_price_prediction, alpha = 1)
coef(pricelasso, s = c(0, 0.01, 0.1, 1))
```

With lambda = 1, it only used the intercept, 0.1 it had a few variables, and manual transmission had a large effect, for lambda = .01, it has more variables and diesel fuel has a higher effect than for other lambdas.
...
```{r}
#| label: 1 c
priceridge = glmnet(logprice ~ . - name - year - selling_price - km_driven, data = car_price_prediction, alpha = 0)
coef(priceridge, s = c(0, .01, .1, 1))

```

The coefficients for 0 and .01 are extremely close if not equal., and a bunch of coefficients slightly become larger for lambda = 0.1, and for 1, almost all are close to zero except intercept.
...
```{r}
#| label: 1 d
pricenet = glmnet(logprice ~ . - name - year - selling_price - km_driven, 
                  data = car_price_prediction, alpha = 1/3)
coef(pricenet, s = c(0, .01, .1, 1))
```

All of them have most of the prediction power on the intercept for lamba = 1, and the ridge seems to have the most variables in the higher lambda values, and the net elastic seems to be in between the two others.
...
#| label: 1 e
set.seed(98765)
cvfitpricelasso = cv.glmnet(logprice ~ . - name - year - selling_price - km_driven, 
                       data = car_price_prediction, alpha = 1)
cvfitpriceridge = cv.glmnet(logprice ~ . - name - year - selling_price - km_driven, 
                       data = car_price_prediction, alpha = 0)
cvfitpricenet = cv.glmnet(logprice ~ . - name - year - selling_price - km_driven, 
                       data = car_price_prediction, alpha = 1/3)
cvfitpricelasso$lambda.1se
cvfitpriceridge$lambda.1se
cvfitpricenet$lambda.1se
coef(cvfitpricelasso, s = "lambda.1se")
coef(cvfitpriceridge, s = "lambda.1se")
coef(cvfitpricenet, s = "lambda.1se")

```

Compare each model's variables and coefficients. The ridge once again removed no variables. The lasso has a stronger relationship between price and transmission type than the others. The net elastic does seem to be a good balance between the two.
...
#| label: 1 f
#| cache: true
newfunction = function(model, i) {
  cvlam = cv.glmnet(logprice ~ . - name - year - selling_price - km_driven, 
                       data = car_price_prediction[i, ], alpha = 0)$lambda.1se
  as.vector(coef(priceridge, s = cvlam))
}
bootstrapping(car_price_prediction, newfunction, 5000)
summary(pricemod)
```

Im not sure I did that function right, but at least I got some sort of output haha. But my bootstrap has much smaller standard errors, if that was supposed to happen.
...
#| label: 2 b
set.seed(1000)
houseridgecv = cv.glmnet(house_value ~ ., data = housing_train, alpha = 0)
houseridge = glmnet(house_value ~ ., data = housing_train, alpha = 0)
lam = houseridgecv$lambda.min
```
...
```{r}
#| label: 2 c
traindata = model.matrix(house_value ~ ., data = housing_train)
sum(residuals(housemod)^2)
yhats = predict.glmnet(houseridge, traindata[ , -1], s = lam)
sum((traindata[, 1]-yhats)^2)
```

The OLS model has smaller sum of squared errors.
...
```{r}
#| label: 2 d
testdata = model.matrix(house_value ~ ., data = housing_test)
pred1 = predict(housemod, housing_test[, -1])
pred2 = predict.glmnet(houseridge, testdata[, -1], s = lam)
sum((testdata[, 1]-pred1)^2)
sum((testdata[, 1]-pred2)^2)
```

Ridge was better, probably because the OLS overfit to the training data, which is why it did better for that but worse for this.
...
newmod = glmnet(house_value ~ ., data = housing_train, 
                alpha = elastic_model$bestTune$alpha
)
pred3 = predict.glmnet(newmod, testdata[, -1], 
                       s = elastic_model$bestTune$lambda
)
sum((testdata[, 1]-pred3)^2)
```

The SSE is slightly larger than the ridge regression
...
data(ShipAccidents)
ShipAccidents = ShipAccidents |> filter(service != 0)
shipmod = glm(incidents ~ ., data = ShipAccidents, family = "poisson")
```

Incidents are events which can only take whole number values and are probably concentrated near smaller values, which is ideal for a poisson regression.
...
The slope service means that the predicted change of the mean of the poisson distribution changes by a factor of exp(6.421\*10\^-5)=1.000064
...
```{r}
#| label: 3 c
p_resids = residuals(shipmod, type = "pearson")
resid_df = data.frame(p_resids, linear_pred = shipmod$linear.predictors)
ggplot(resid_df, aes(x = linear_pred, y = p_resids)) +
  geom_point(size = 0.5) +
  labs(x = "Linear Predictors", y = "Pearson Residuals")
ggplot(resid_df, aes(sample = p_resids)) + geom_qq() + geom_qq_line() +
  labs(x = "Theoretical Quantiles", y = "Sample Quantiles")
```

It seems to be not the best fit, the residuals seem very right skewed.
...
#| label: 3 d
shipmod$deviance
1 - pchisq(shipmod$deviance, df = shipmod$df.residual)
```

H_0: The model is correctly specified, H_a: The model is incorrectly specified. D = 77.4498, p-value = 1.542447e-07 Therefore we have very strong evidence that the model is incorrectly specified and is not a good fit.
...
```{r}
#| label: 3 e
sum(residuals(shipmod, type = "pearson")^2)/shipmod$df.residual
```

The model is probably overdispersed, the dispersion value was 2.896, so it is most likely overdispersed.
...
```{r}
#| label: 3 f
shipmodquasi = glm(incidents ~ ., data = ShipAccidents, family = "quasipoisson")
summary(shipmodquasi)
shippred = predict(shipmodquasi, 
                   data.frame(type = "B", construction = "1965-69", 
                              operation = "1960-74", service = 2000), 
                   se.fit = T)
exp(shippred$fit - qt(.975, 24)*shippred$se.fit)
exp(shippred$fit + qt(.975, 24)*shippred$se.fit)
```

With 95% confidence, the average incident value given a B ship built between 1965-69, operated between 1960-74, and had 2000 service months will be between 3.6623 and 19.9458.
...
We are 95% confident that the number of incidents given a B ship built between 1965-69, operated between 1960-74, and with 2000 months of service will be between 0 and 29.
...
```{r}
#| label: 3 h
shippred2 = predict(shipmod, 
                   data.frame(type = "B", construction = "1965-69", 
                              operation = "1960-74", service = 2000), 
                   se.fit = T)
exp(shippred2$fit - 1.96*shippred2$se.fit)
exp(shippred2$fit + 1.96*shippred2$se.fit)
qpois(.025, exp(shippred2$fit - 1.96*shippred2$se.fit))
qpois(.975, exp(shippred2$fit + 1.96*shippred2$se.fit))
```

Both intervals are more narrow than the quasi model.
