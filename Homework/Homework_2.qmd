...
#| eval: false
library(inborutils)
csv_to_sqlite(csv_file = "/Users/prestonhindman/Math_3190/Data/OneDrive_1_2-1-2026/airlines.csv", 
              sqlite_file = "combineddata.sqlite",
              table_name = "AIRLINES")
csv_to_sqlite(csv_file = "/Users/prestonhindman/Math_3190/Data/OneDrive_1_2-1-2026/airports.csv", 
              sqlite_file = "combineddata.sqlite",
              table_name = "AIRPORTS")
csv_to_sqlite(csv_file = "/Users/prestonhindman/Math_3190/Data/OneDrive_1_2-1-2026/flights.csv", 
              sqlite_file = "combineddata.sqlite",
              table_name = "FLIGHTS")
```
...
```{r}
#| label: Connection
flightcon = DBI::dbConnect(RSQLite::SQLite(),
                           "combineddata.sqlite")
DBI::dbListTables(flightcon)
```
...
```{r}
#| label: Flights Columns
DBI::dbGetQuery(flightcon, "PRAGMA table_info(flights)")
```
...
#| label: Queries
# 1
DBI::dbGetQuery(flightcon, "SELECT AVG(DISTANCE) AS 'Average Distance'
                FROM FLIGHTS 
                WHERE YEAR = 2015")
# 2
DBI::dbGetQuery(flightcon, "SELECT MIN(DISTANCE) AS 'Minimum Distance'
                FROM FLIGHTS 
                WHERE YEAR = 2015")
# 3
DBI::dbGetQuery(flightcon, "SELECT MAX(DISTANCE) AS 'Maximum Distance'
                FROM FLIGHTS 
                WHERE YEAR = 2015")
# 4
DBI::dbGetQuery(flightcon, "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT
                FROM FLIGHTS
                WHERE DISTANCE = (SELECT MIN(DISTANCE) FROM FLIGHTS)")
# 5
DBI::dbGetQuery(flightcon, "SELECT DISTINCT ORIGIN_AIRPORT, DESTINATION_AIRPORT
                FROM FLIGHTS
                WHERE DISTANCE = (SELECT MAX(DISTANCE) FROM FLIGHTS)")
# 6
DBI::dbGetQuery(flightcon, "SELECT DISTINCT ORIGIN_AIRPORT, DESTINATION_AIRPORT
                FROM FLIGHTS
                WHERE DISTANCE = (SELECT MAX(DISTANCE) FROM FLIGHTS) AND
                ORIGIN_AIRPORT NOT LIKE '1%' AND 
                DESTINATION_AIRPORT NOT LIKE '1%'")
# 7
DBI::dbGetQuery(flightcon, "SELECT AIRPORTS.AIRPORT 
                FROM AIRPORTS
                INNER JOIN FLIGHTS
                ON AIRPORTS.IATA_CODE IN  (FLIGHTS.ORIGIN_AIRPORT,
                FLIGHTS.DESTINATION_AIRPORT)
                WHERE FLIGHTS.DISTANCE = (SELECT MIN(DISTANCE) FROM FLIGHTS)")
# 8
DBI::dbGetQuery(flightcon, "SELECT DISTINCT AIRPORTS.AIRPORT 
                FROM AIRPORTS
                INNER JOIN FLIGHTS
                ON AIRPORTS.IATA_CODE IN  (FLIGHTS.ORIGIN_AIRPORT,
                FLIGHTS.DESTINATION_AIRPORT)
                WHERE FLIGHTS.DISTANCE = (SELECT MAX(DISTANCE) FROM FLIGHTS)")
```
...
```{r}
#| label: R tibble from SQL
library(tidyverse)
flightdelays = as_tibble(DBI::dbGetQuery(flightcon, "SELECT FLIGHTS.ARRIVAL_DELAY, AIRLINES.AIRLINE
                FROM FLIGHTS
                INNER JOIN AIRLINES
...
```{r}
#| label: GGplot
ggplot(data = flightdelays, aes(x = AIRLINE, y = ARRIVAL_DELAY)) +
  geom_boxplot() +
  scale_y_log10() +
...
```{sql}
--| connection: flightcon
--| output.var: avgdelay
SELECT AIRLINES.AIRLINE, AVG(FLIGHTS.ARRIVAL_DELAY) AS "avg_delay"
FROM FLIGHTS
INNER JOIN AIRLINES
ON FLIGHTS.AIRLINE = AIRLINES.IATA_CODE
...
```{r}
#| label: Barplot
ggplot(data = avgdelay, aes(x = AIRLINE, y = avg_delay)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
#| label: tbl
flighttbl = tbl(flightcon, "flights")
airporttbl = tbl(flightcon, "airports")
airlinestbl = tbl(flightcon, "airlines")
...
```{r}
#| label: tidyverse
timesummary = flighttbl |> 
  group_by(AIRLINE) |> 
  summarise(AVG_TIME = mean(ELAPSED_TIME, na.rm = T), SD_TIME = sd(ELAPSED_TIME, na.rm = T), .groups = "drop") |> 
    inner_join(airlinestbl, by = c("AIRLINE" = "IATA_CODE")) |> 
...
```{r}
#| label: gg bar
ggplot(data = timesummary, aes(x = AIRLINE.y, y = SD_TIME)) +
  geom_col() +
   theme(axis.text.x = element_text(angle = 45, hjust = 1))

DBI::dbDisconnect(flightcon)
